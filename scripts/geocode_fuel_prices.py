...
def geocode_address(name, city, state):
    ...
    """
    headers = {
        'User-Agent': 'FuelPriceGeocoder/1.0 (waseemriyaz718@gmail.com)'
    }
...
def main():
    ...
    logging.info(f"Geocoding fuel prices from '{input_csv_file}' to '{output_csv_file}'")
    ...
    try:
        with open(input_csv_file, 'r', newline='') as infile, \
            ...
            for row in reader:
                rows_processed += 1
                truckstop_id, truckstop_name, address, city, state, rack_id, retail_price = row
                ...
                if latitude and longitude:
                    ...
                    logging.info(f"Geocoded {truckstop_name} in {city}, {state} to ({latitude}, {longitude})")
                    rows_geocoded += 1
                else:
                    writer.writerow(row + ['', ''])
                    logging.info(f"Could not geocode {truckstop_name} in {city}, {state}")
